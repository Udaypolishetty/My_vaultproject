...
const canRequestVp = !club.vpRoll;
  ...
  const handlePresidentRemove = async (rollNumber) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/clubs/${club.id}/president-remove-member`, {
        method: "POST",
  ...
  const handleAdminRemove = async (rollNumber) => {
    setRemoving(rollNumber);
    try {
      const res = await fetch(`${API_BASE}/api/clubs/${club.id}/admin-remove-member`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ rollNumber })
      });
  ...
  const handleSaveNick = async (rollNumber) => {
    ...
    setSavingNick(true);
    try {
      const res = await fetch(`${API_BASE}/api/clubs/${club.id}/set-nickname`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
