
import { API_BASE } from "../../config/api";
import { useState } from "react";
import {
  Send,
  BookOpen,
  SearchCheck,
  Briefcase,
  CalendarDays,
  MessageSquare,
  Globe,
  GraduationCap,
  Building2,
  Target,
  AlertTriangle,
} from "lucide-react";
import { validateBuzzContent } from "../../utils/validate";

const TAG_CONFIG = {
  STUDY_GROUP: {
    label: "Study Group",
    icon: BookOpen,
    color: "bg-blue-500/20 text-blue-400 border-blue-500/30",
  },
  LOST_FOUND: {
    label: "Lost & Found",
    icon: SearchCheck,
    color: "bg-yellow-500/20 text-yellow-400 border-yellow-500/30",
  },
  OPPORTUNITY: {
    label: "Opportunity",
    icon: Briefcase,
    color: "bg-purple-500/20 text-purple-400 border-purple-500/30",
  },
  EVENT: {
    label: "Event",
    icon: CalendarDays,
    color: "bg-pink-500/20 text-pink-400 border-pink-500/30",
  },
  GENERAL: {
    label: "General",
    icon: MessageSquare,
    color: "bg-gray-500/20 text-gray-400 border-gray-500/30",
  },
};

const VISIBILITY_CONFIG = [
  {
    value: "EVERYONE",
    label: "Everyone",
    icon: Globe,
  },
  {
    value: "MY_YEAR",
    label: "My Year",
    icon: GraduationCap,
  },
  {
    value: "MY_BRANCH",
    label: "My Branch",
    icon: Building2,
  },
  {
    value: "MY_YEAR_BRANCH",
    label: "My Year & Branch",
    icon: Target,
  },
];

export default function BuzzComposer({ token, onPost }) {
  const [content, setContent] = useState("");
  const [tag, setTag] = useState("GENERAL");
  const [visibility, setVisibility] = useState("EVERYONE");
  const [posting, setPosting] = useState(false);
  const [error, setError] = useState("");

  const handlePost = async () => {
    if (posting) return;

    const result = validateBuzzContent(content);
    if (!result.valid) {
      setError(result.error);
      return;
    }

    setPosting(true);
    setError("");

    try {
      const res = await fetch(`${API_BASE}/api/buzz/create`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ content, tag, visibility }),
      });

      if (!res.ok) return;

      const newPost = await res.json();
      onPost(newPost);
      setContent("");
      setVisibility("EVERYONE");
      setTag("GENERAL");
    } catch (err) {
      setError("Failed to post. Please try again.");
    } finally {
      setPosting(false);
    }
  };

  return (
    <div
      className="bg-gradient-to-br from-[#111] to-[#0d0d0d] border border-white/10
                    rounded-2xl p-5 mb-4 shadow-lg"
    >
      <div className="flex items-start gap-3">
        <div
          className="w-10 h-10 rounded-full bg-[#26F2D0]/20 border border-[#26F2D0]/30
                        flex items-center justify-center text-[#26F2D0] font-bold text-sm shrink-0"
        >
          {sessionStorage.getItem("name")?.[0]?.toUpperCase() || "?"}
        </div>

        <div className="flex-1">
          <textarea
            value={content}
            onChange={(e) => {
              setContent(e.target.value);
              if (error) setError("");
            }}
            placeholder="What's on your mind? Share with campus..."
            maxLength={280}
            rows={3}
            className={`w-full bg-[#1a1a1a] text-white placeholder-gray-500 text-sm
                       resize-none outline-none leading-relaxed rounded-xl p-3
                       border transition
                       ${
                         error
                           ? "border-red-500/50 focus:border-red-500"
                           : "border-white/10 focus:border-[#26F2D0]/50"
                       }`}
          />

          {error && (
            <p className="text-xs text-red-400 flex items-center gap-1 mt-2 pl-3">
              <AlertTriangle size={12} />
              <span>{error}</span>
            </p>
          )}

          {/* Tag selector */}
          <div className="mt-3">
            <p className="text-xs text-gray-500 mb-2">Category</p>
            <div className="flex gap-2 flex-wrap">
              {Object.entries(TAG_CONFIG).map(([key, val]) => {
                const TagIcon = val.icon;

                return (
                  <button
                    key={key}
                    onClick={() => setTag(key)}
                    className={`text-xs px-3 py-1 rounded-full border transition-all flex items-center gap-1.5
                    ${
                      tag === key
                        ? `${val.color} font-semibold`
                        : "border-white/10 text-gray-500 hover:border-white/30"
                    }`}
                  >
                    <TagIcon size={13} />
                    <span>{val.label}</span>
                  </button>
                );
              })}
            </div>
          </div>

          <div className="border-t border-white/10 my-3" />

          {/* Visibility selector */}
          <div>
            <p className="text-xs text-gray-500 mb-2 flex items-center gap-1.5">
              <Globe size={12} />
              <span>Who can see this?</span>
            </p>

            <div className="flex gap-2 flex-wrap">
              {VISIBILITY_CONFIG.map((opt) => {
                const VisibilityIcon = opt.icon;

                return (
                  <button
                    key={opt.value}
                    onClick={() => setVisibility(opt.value)}
                    className={`text-xs px-3 py-1 rounded-full border transition-all flex items-center gap-1.5
                    ${
                      visibility === opt.value
                        ? "bg-[#26F2D0]/20 text-[#26F2D0] border-[#26F2D0]/30"
                        : "border-white/10 text-gray-500 hover:border-white/30"
                    }`}
                  >
                    <VisibilityIcon size={13} />
                    <span>{opt.label}</span>
                  </button>
                );
              })}
            </div>
          </div>

          <div className="flex items-center justify-end gap-3 mt-3">
            <span className={`text-xs ${content.length > 250 ? "text-red-400" : "text-gray-500"}`}>
              {content.length}/280
            </span>

            <button
              onClick={handlePost}
              disabled={posting}
              className="flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold
                         bg-[#26F2D0] text-black hover:bg-[#1dd4b8] transition
                         disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Send size={13} />
              {posting ? "Posting..." : "Post"}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

export { TAG_CONFIG };