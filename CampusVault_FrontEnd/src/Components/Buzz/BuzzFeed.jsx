...
export default function BuzzFeed({
  ...
  const hotPosts = [...posts]
    .filter((p) => {
      ...
      return (
        ageHours < 24 &&
        ((p.likes || 0) + (p.replies?.length || 0)) > 0 &&
        !p.resolved
      );
    })
    .sort((a, b) => getScore(b) - getScore(a))
  ...
  const currentTag = TAG_CONFIG[filterTag];
  const CurrentTagIcon = currentTag?.icon;
    ...
    <div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Search posts..."
          className="w-full bg-[#111] border border-white/10 rounded-full pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-[#26F2D0]/50 transition"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
        <button
          ...
          className={`text-xs px-4 py-1.5 rounded-full border whitespace-nowrap transition-all flex items-center gap-1.5 ${
            filterTag === "ALL"
              ? "bg-[#26F2D0]/20 text-[#26F2D0] border-[#26F2D0]/30"
              : "border-white/10 text-gray-500 hover:border-white/30"
          }`}
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className="text-sm font-semibold text-gray-400">
          {search ? (
            ...
            <span className="flex items-center gap-1.5">
              {CurrentTagIcon && <CurrentTagIcon size={14} />}
