...
/* ─────────────────────────────────────────────
   Rate limit keyed per student roll number.
   Key format: vault_join_sent_<ROLLNUMBER>
   This means each student's cooldown is tracked
   independently — one submission does NOT lock
   other students on the same device.
───────────────────────────────────────────── */
function getRLKey() {
  const roll = sessionStorage.getItem("rollNumber") || "guest";
  return `vault_join_sent_${roll}`;
}
function getLastSent() {
  try { return localStorage.getItem(getRLKey()); } catch { return null; }
}
function saveLastSent() {
  try { localStorage.setItem(getRLKey(), new Date().toISOString()); } catch {}
}
...
const About = () => {
  /* ── form state — name + message only, no email ── */
  const [form,      setForm]      = useState({ name: "", message: "" });
  const [cardMode,  setCardMode]  = useState("join");
  const [sendState, setSendState] = useState("idle");
  const [errMsg,    setErrMsg]    = useState("");
  const [countdown, setCountdown] = useState(() => cooldownRemaining());
  const [copied,    setCopied]    = useState(false);

  /* re-check cooldown when component mounts (handles login switch) */
  useEffect(() => {
  ...
  /* ── send via EmailJS — no email field ── */
  const handleSubmit = async (e) => {
    ...
    const roll = sessionStorage.getItem("rollNumber") || "Unknown";
    ...
    try {
      await emailjs.send(
        ...
        EJ_TEMPLATE,
        {
          name:       "Campus Vault Admin",
          from_name:  form.name.trim(),
          from_email: `${roll}@campusvault`,   // roll number as identifier
          message:    form.message.trim(),
          time:       new Date().toLocaleString("en-IN", {
                        dateStyle: "medium", timeStyle: "short",
                      }),
          reply_to:   `${roll}@campusvault`,
        },
        EJ_PUBLIC
      );
  ...
  return (
    <div className="bg-[#0b0b0b] text-white relative min-h-screen overflow-hidden font-sans">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div className="space-y-4">
              {[
                ...
                "Shared Academic Resources/Notes",
                "Idea Exchange Hub",
      ...
      <section className="py-20 flex flex-col items-center px-6 text-center z-10 relative">
        ...
        <div className="flex flex-col md:flex-row gap-12 justify-center">
          {[
            ...
            { name: "Adepu Shiva",     role: "Connect", url: "https://www.linkedin.com/in/adepu-shiva-5656b1301" },
          ].map((person, i) => (
            <a key={i} href={person.url} target="_blank" rel="noopener noreferrer"
              className="relative group w-full max-w-[280px] md:w-72">
        ...
        <div className="max-w-2xl w-full bg-white/5 border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-md">
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow:  cardMode === opt.key
                      ? "0 0 16px rgba(38,242,208,0.35)" : "none",
                  }}
                >
                  {opt.label}
                </button>
          ...
          {cardMode === "join" && (
            <div className="fade-in">
              ...
              {/* Form — name + message only, NO email field */}
              {!isLocked && sendState !== "success" && (
      ...
      <footer className="py-20 flex flex-col items-center text-center px-6 border-t border-white/5 z-10 relative">
        ...
        <h1
          className="text-6xl md:text-9xl font-black tracking-tighter opacity-10 absolute -bottom-5 select-none"
